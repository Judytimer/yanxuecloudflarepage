...
export default function ChatWindow({ onClose, initialQuestion }: ChatWindowProps) {
        ...
        setMessages([
          {
            id: crypto.randomUUID(),
            role: 'assistant',
            content: '你好！我是你的AI家庭教育顾问。有什么家庭教育方面的困扰，都可以告诉我。',
            timestamp: new Date(),
          },
        ]);
      }
  ...
  // 如果有初始问题，自动发送
  useEffect(() => {
    if (initialQuestion && !hasSentInitialQuestion.current && messages.length > 0) {
  ...
  }, [initialQuestion, messages, handleSendMessage]);
